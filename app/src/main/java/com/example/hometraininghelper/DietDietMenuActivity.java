...
/* 해당 액티비티 클래스는 웨이트 운동 부위 중에 다이어트에 해당하는 식단 부분이다.
이 클래스는 특별한 동작 없이 단순히 다이어트의 식단에 해당되는 레이아웃을 연결하여 관련된 유튜브 영상을 볼 수 있도록하며,
...
public class DietDietMenuActivity extends Activity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    /* 해당 onCreate 메서드는 다이어트의 식단에 해당되는 영상을 보여주는 레이아웃을 설정하여, 해당 영상을 볼 수 있도록한다.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_diet_diet_menu); // 현재 액티비티의 레이아웃을 activity_diet_diet_menu.xml로 설정한다.
