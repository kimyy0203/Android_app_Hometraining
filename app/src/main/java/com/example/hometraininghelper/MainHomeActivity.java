...
/* 해당 액티비티 클래스는 홈 트레이닝 화면 구성의 동작을 구현한 클래스이다. 홈트에 해당되는 여러 운동 부위들의 이미지 버튼을 생성하여,
...
public class MainHomeActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    ImageButton btn_shoulder; // 어깨 페이지
    ImageButton btn_arm; // 팔 페이지
    ImageButton btn_abs; // 복근 페이지
    ImageButton btn_chest; // 가슴 페이지
    ImageButton btn_leg; // 다리 페이지
    ImageButton btn_stretch; // 스트레칭 페이지

    /* 해당 onCreate 메서드는 사용자가 다양한 이미지 버튼을 클릭할 때 다른 액티비티로 이동하는 기능을 구현하였다.
    이를 통해 이미지 버튼 클릭 이벤트를 처리하여 각각의 운동 단계(초급, 중급, 고급)로 이동할 수 있다.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_main_home); // 현재 액티비티의 레이아웃을 activity_main_home.xml로 설정한다.
