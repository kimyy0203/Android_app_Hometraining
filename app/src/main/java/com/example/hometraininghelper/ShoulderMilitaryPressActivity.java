...
/* 해당 액티비티 클래스는 웨이트 운동 부위 중에 어깨에 해당하는 밀리터리 프레스 부분이다.
이 클래스는 특별한 동작 없이 단순히 어깨의 밀리터리 프레스에 해당되는 레이아웃을 연결하여 관련된 유튜브 영상을 볼 수 있도록하며,
드로어 메뉴를 생성하고 설정하여, 이 해당 화면에서도 다른 운동 부위로 넘어가거나 다른 옵션 메뉴를 사용할 수 있도록 한다. */
public class ShoulderMilitaryPressActivity extends Activity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    /* 해당 onCreate 메서드는 어깨의 밀리터리 프레스에 해당되는 영상을 보여주는 레이아웃을 설정하여, 해당 영상을 볼 수 있도록한다.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_shoulder_military_press); // 현재 액티비티의 레이아웃을 activity_shoulder_military_press.xml로 설정한다.
