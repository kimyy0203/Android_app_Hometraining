...
/* 해당 액티비티 클래스는 홈트 운동 부위 중에 스트레칭에 해당하는 부분이다. 이 액티비티를 통해 스트레칭의 초급, 중급, 고급 운동으로 넘어갈 수 있다.
또한 드로어 메뉴를 통해 메인 메뉴 화면이 아닌 다른 화면에서도 다른 운동 부위로 넘어갈 수 있도록 하는 동작과
또 다른 옵션 메뉴(검색 기능과 같은 옵션)를 사용할 수 있도록 구현하였다. */
public class StretchActivity extends Activity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    ImageButton btn_whole; // 전신 스트레칭
    ImageButton btn_neck_shoulder; // 목 및 어깨 스트레칭
    ImageButton btn_upper; // 상체(팔 어깨 가슴) 스트레칭
    ImageButton btn_lower; // 하체 스트레칭

    /* 해당 onCreate 메서드는 사용자가 다양한 버튼을 클릭할 때 다른 액티비티로 이동하는 기능을 구현하였다.
    이를 통해 버튼 클릭 이벤트를 처리하여 각각의 스트레칭 종류(전신, 목 및 어깨, 상체, 하체로 이동할 수 있다.
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        setContentView(R.layout.activity_stretch); // 현재 액티비티의 레이아웃을 activity_stretch.xml로 설정한다.
        ...
        btn_whole = findViewById(R.id.basic); // 스트레칭 전신

        btn_whole.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getApplicationContext(), StretchWholeActivity.class);
                startActivity(intent);
            }
        });

        btn_neck_shoulder = findViewById(R.id.nor); // 스트레칭 목 및 어깨

        btn_neck_shoulder.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getApplicationContext(), StretchNeckShoulderActivity.class);
                startActivity(intent);
            }
        });

        btn_upper = findViewById(R.id.high); // 스트레칭 상체

        btn_upper.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getApplicationContext(), StretchUpperActivity.class);
                startActivity(intent);
            }
        });
        btn_lower = findViewById(R.id.leg); // 스트레칭 하체

        btn_lower.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getApplicationContext(), StretchLowerActivity.class);
                startActivity(intent);
            }
        });
    }
