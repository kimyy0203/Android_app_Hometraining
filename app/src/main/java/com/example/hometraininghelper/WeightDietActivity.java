...
/* 해당 액티비티 클래스는 웨이트 운동 부위 중에 다이어트에 해당하는 부분이다. 이 액티비티를 통해 다이어트로 넘어갈 수 있다.
또한 드로어 메뉴를 통해 메인 메뉴 화면이 아닌 다른 화면에서도 다른 운동 부위로 넘어갈 수 있도록 하는 동작과
또 다른 옵션 메뉴(검색 기능과 같은 옵션)를 사용할 수 있도록 구현하였다. */
public class WeightDietActivity extends Activity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    이를 통해 버튼 클릭 이벤트를 처리하여 다이어트 방식 중 하나로 이동할 수 있다.
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        setContentView(R.layout.activity_diet_weight);  // 현재 액티비티의 레이아웃을 activity_diet_weight.xml로 설정한다.
        ...
        btn_1 = findViewById(R.id.diet_menu); // 식단

        btn_1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getApplicationContext(), DietDietMenuActivity.class);
